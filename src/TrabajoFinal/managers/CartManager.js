...
export default class CartManager {
  ...
  async findCartsById(CartsId) {
    try {
      const carts = await this.getAllCarts();
      let cartFind = carts.find((cartIterated) => cartIterated.id === CartsId);
      if (cartFind) {
        return cartFind;
      } else {
        cartFind = await this.generateCart(CartsId);
        return cartFind;
      }
    } catch (error) {
      console.log(error);
    ...
    try {
      ...
      if (findCart) {
        ...
        const findProduct = products.find(
          (prodIterated) => prodIterated.id === productId
        );
        if (findProduct) {
          return findProduct;
        } else {
  ...
  async addProductsCarts(cartId, idProduct) {
    try {
      const productFindInProducts =
        await productManager.findProductByIdProducts(idProduct);
      ...
      if (productFindInProducts) {
        ...
        if (productFound === null) {
          ...
          cartToAdd.products = cartToAdd.products.filter(
